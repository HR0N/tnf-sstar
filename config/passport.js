...
const Web3Strategy = passportCustom.Strategy;
passport.use('web3', new Web3Strategy((req, done) => {
  try {
    req.query.address = ethers.utils.getAddress(req.query.address);
  } catch (e) {
    done(e);
  }

  const address = ethers.utils.recoverAddress(req.query.digest, req.query.signature);
  if (address !== req.query.address) {
    return done({ msg: 'Invalid signer.' });
  }

  if (req.user) {
    User.findOne({ address }, (err, existingUser) => {
      if (err) { return done(err); }
      if (existingUser) {
        req.flash('errors', { msg: 'There is already a MetaMask account that belongs to you. Sign in with that account or delete it, then link it with your current account.' });
        done(err);
      } else {
        User.findById(req.user.id, (err, user) => {
          ...
          user.save((err) => {
            req.flash('info', { msg: 'MetaMask account has been linked.' });
            done(err, user);
          });
        });
      }
    ...
    User.findOne({ address }, (err, existingUser) => {
      ...
      user.email = `${address}@metamask.io`;
      user.address = address;
      user.save((err) => {
        done(err, user);
      });
